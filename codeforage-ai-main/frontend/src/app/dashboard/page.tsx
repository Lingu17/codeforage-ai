  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      const activeRepos = scannedRepos.filter((repo) => {
        ...
        const repoStatus = repo.status || "completed";
        return !isTerminal(repoStatus);
      });
        ...
        try {
          ...
          setJobStatuses(prev => {
            ...
            if (shouldFetchSummary) {
              fetchGlobalStats();
              if (repo.id === selectedRepoIdRef.current) {
                fetchRepoSummary(repo.id);
              }
            }

            // Track real per-stage states for this repository
            setStageStatuses((prevStages) => ({
              ...prevStages,
              [repo.id]: parseStages(statusData),
            }));
